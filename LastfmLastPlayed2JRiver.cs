...
class Script : MarshalByRefObject
{
  public void Init(MediaCenter.MCAutomation mediaCenterInterface)
  {
    ...
    // Number of tracks to fetch per page (maximum 200)
    int numTracks = 200;
    // Max number of pages to fetch (set to 0 to fetch all pages and tracks, otherwise only the [maxPages*numTracks] most recent tracks will be fetched)
    int maxPages = 1;

    LastFM lfm = new LastFM();
    Console.WriteLine("Fetching recent tracks for user '" + username + "' from Last.fm ...");
    RecentTrack[] recentTracks = lfm.getRecentTracks(username, numTracks, maxPages);
    ...
    foreach (RecentTrack track in recentTracks)
    {
      ...
      DateTime lfmLastPlayed = new DateTime(1970, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc);
      lfmLastPlayed = lfmLastPlayed.AddSeconds(Int64.Parse(track.Date));
      lfmLastPlayed = lfmLastPlayed.ToLocalTime();
      ...
      for(int i = 0; i < localTracks.GetNumberFiles(); i++)
      {
        ...
        {
          DateTime localLastPlayed = new DateTime(1970, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc);
          localLastPlayed = localLastPlayed.AddSeconds(localTrackTime);
          localLastPlayed = localLastPlayed.ToLocalTime();
          string localLastPlayedString;
          if (localTrackTime>0)
          {
            localLastPlayedString = localLastPlayed.ToString();
          }
          else
          {
            localLastPlayedString = "never";
          }
          Console.WriteLine("Updating " + track.Artist + " - " + track.Name + " - " + track.Album + " (Local last played: " + localLastPlayedString + " Last.fm last played: " + lfmLastPlayed.ToString() + ")\t\t");
