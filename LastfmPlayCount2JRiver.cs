...
class Script : MarshalByRefObject
{
  public void Init(MediaCenter.MCAutomation mediaCenterInterface)
  {
    ...
    // Number of tracks to fetch per page (all pages will be fetched)
    int numTracks = 200;

    LastFM lfm = new LastFM();
    Console.WriteLine("Fetching library tracks for user '" + username + "' from Last.fm ...");
    LibraryTrack[] libraryTracks = lfm.getLibraryTracks(username, numTracks);

    Console.WriteLine("Matching tracks from Last.fm to tracks in JRiver Media Center ...");
    foreach (LibraryTrack track in libraryTracks)
    {
